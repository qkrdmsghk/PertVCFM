...
class PertDataModule(L.LightningDataModule):
    ...
    def __init__(
        ...
        p_emb_key: str = None, # scGenePT or GenePert or GenePertESM
        mask_degs: str = None,
        ...
        self.p_emb_key = p_emb_key
        self.mask_degs = mask_degs
        ...
        if self.mask_degs != 'None':
            self.mask_degs = pickle.load(open(f"AAA_DEGs/{self.task_type}/true_degs_dict.pkl", "rb"))

        if self.p_emb_key == "scGenePT":
            print("Loading scGenePT embeddings")
            self.p_emb_dict = pickle.load(open("/home/yinhua/CellPertQA/scGenePT/models/gene_embeddings/GO_all_gene_embeddings-gpt3.5-ada-concat.pickle", "rb"))
        elif self.p_emb_key == "GenePert":
            print("Loading GenePert embeddings")
            self.p_emb_dict = pickle.load(open("GenePT_emebdding_v2/GenePT_gene_embedding_ada_text.pickle", "rb"))
        elif self.p_emb_key == "GenePertESM":
            print("Loading GenePertESM embeddings")
            self.p_emb_dict = pickle.load(open("/home/yinhua/CellPertQA/TxPert/GenePT_emebdding_v2/GenePT_gene_protein_embedding_model_3_text.pickle.", "rb"))
        else:
            print("No perturbation embedding key provided")
            self.p_emb_key = None
            self.p_emb_dict = None
            self.p_emb_dim = 0
    ...
    def prepare_data(self, seed: int = 1) -> None:

        if self.data_prepared:
            return

        cache_file = f"{self.cache_path}/de_adata_test.h5ad" # de_adata_test_layer.h5ad
        ...
        if os.path.exists(cache_file):
            ...
            logger.info(f"Loading DEGs to uns")
            if "rank_genes_groups_cov_all" not in self.adata.uns:
                self.adata = self.load_degs_to_uns(self.adata, deg_dir=f'AAA_DEGs/{self.task_type}/true_degs')

        else:
            raise ValueError(f"Unsupported dataset.")

        self._split_data()
        self._load_predefined_splits(seed)
    ...
    def _set_input_output_dimensions(self) -> None:
        ...
        if self.p_emb_key is not None:
            self.p_emb_dim = len(self.p_emb_dict['AARS1'])
        else:
            self.p_emb_dim = 0
    ...
    def _load_predefined_splits(self, seed: int = 1) -> None:
        """
        ...
        if seed == 0:
            seed_split_path = self.cache_path / f"splits/predefined/train_test_split.pkl"
            seed_subgroup_path =self.cache_path / f"splits/predefined/subgroup.pkl"
        else:
            seed_split_path = self.cache_path / f"splits/seed_{seed}/train_test_split.pkl"
            seed_subgroup_path =self.cache_path / f"splits/seed_{seed}/subgroup.pkl"
        if not os.path.exists(
            seed_split_path) or not os.path.exists(seed_subgroup_path):
            print(seed_split_path)
            raise ValueError("Predefined splits not found")
        self.condition_group = joblib.load(seed_split_path)
        self.subgroup = joblib.load(seed_subgroup_path)
        logger.info(f"Using predefined splits --> {seed_split_path}")
    ...
    def _create_datasets(self) -> None:
        ...
        self.train_data = XcelltypePerturbDataset(
            ...
            p_emb_key=self.p_emb_key,
            p_emb_dict=self.p_emb_dict,
        )
    ...
    def train_dataloader(self) -> DataLoader:
        ...
        return DataLoader(
            ...
            drop_last=True,
            num_workers=4,          # 如果 CPU 核多，可以试 8
            pin_memory=True,        # GPU 训练一定建议开
            persistent_workers=True # 多个 epoch 不要重复创建 worker
        )
...
class XcelltypePerturbDataset(Dataset):
    ...
    def _to_tensor(self, data: csr_matrix) -> torch.Tensor:
        ...
        """
        return torch.Tensor(data.toarray())#.to("cuda")
    ...
    def __getitem__(self, idx: int) -> Tuple[
        ...
        name = self.condition_names.iloc[idx].split('+')[0].split('_')[1]
        if self.p_emb_key is None:
            p_emb = torch.zeros(2)
        else:
            if name in self.p_emb_dict:
                p_emb = torch.tensor(self.p_emb_dict[name])
            else:
                p_emb = torch.tensor(self.p_emb_dict['AARS1'])

        mask_degs = torch.ones(pert_data.shape[0])
        if self.mask_degs != 'None':
            if name in self.mask_degs:
                mask_degs = torch.zeros(pert_data.shape[0])
                mask_degs[self.mask_degs[name]] = 1
        mask_degs = mask_degs.bool()
        return (
            pert_data,
            control,
            p_emb,
    ...
    def _get_control_data(self, idx: int) -> torch.Tensor:
        ...
        if self.match_cntr:
            if self.avg_cntr:
                ...
                # random sample one control
                # control = self.control_data[
                #     np.random.choice(self.control_ct_batch_indices[cell_type][batch])
                # ]
                
                # controlmean + random noise

                control = self.control_data[self.control_ct_batch_indices[cell_type][batch], :]  # (n_ctrl, d)
                gene_std = control.std(dim=0, unbiased=False)
                eps = 1e-6
                gene_std = gene_std.clamp_min(eps)
                alpha = 1 # 0.1
                noise = torch.randn_like(control.mean(0)) * alpha * gene_std  # (n_ctrl, d)
                control = control.mean(0) + noise
        else:
            if self.avg_cntr:
