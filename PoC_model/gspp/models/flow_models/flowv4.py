...
#                 use_layer_norm: bool = False): # 增加 LN 选项
...
class Flowv4(nn.Module):
    def __init__(self, 
        ...
        super(Flowv4, self).__init__()
        
        self.cond = cond
        self.hidden_dim = hidden_dim

        # 各分支 Encoder
        self.x_encoder = MLPBlock(dims=[input_dim, hidden_dim*2, hidden_dim], dropout_rate=0.1, use_layer_norm=True)
        self.time_encoder = MLPBlock(dims=[hidden_dim, hidden_dim*2, hidden_dim], dropout_rate=0, use_layer_norm=True)
        ...
        base_dim = hidden_dim + hidden_dim + cond_dim
        
        if self.cond == "no_control":
            self.flow_input_dim = base_dim
        elif self.cond == "control_mean":
            self.flow_input_dim = base_dim + hidden_dim
        elif self.cond == "noised_control_mean":
            self.flow_input_dim = base_dim + hidden_dim + hidden_dim
        else:
        ...
        # 速度预测网络
        self.velocity_net = MLPBlock(
            dims=[self.flow_input_dim, hidden_dim*2, hidden_dim, output_dim], 
            dropout_rate=dropout
        )

    def forward(self, x_t, t, conditions, x_0_instrinsic=None, noise=None):
        # 1. 对各个输入进行编码
        x_t_enc = self.x_encoder(x_t)
        t_enc = self.time_encoder(timestep_embedding(t*1000, self.hidden_dim))
        ...
        # 3. 按条件注入 Control 信息
        if self.cond in ["control_mean", "noised_control_mean"]:
            if x_0_instrinsic is not None:
                x_0_enc = self.x_0_encoder(x_0_instrinsic)
                feat_list.append(x_0_enc)
        
        # 4. 按条件注入额外 Noise 信息
        if self.cond == "noised_control_mean":
            if noise is not None:
                n_enc = self.noise_encoder(noise)
                feat_list.append(n_enc)
