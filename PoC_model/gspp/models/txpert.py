...
class TxPert(nn.Module):
    def __init__(
        ...
        Args:
            ...
            p_emb_dim (int): Dimension of the perturbation embedding.
        ...
        # self.ln_pert = nn.LayerNorm(latent_dim)       
        decoder_layers_intrinsic = [
            nn.Linear(latent_dim + (self.input_dim - self.output_dim) + latent_dim + self.p_emb_dim, hidden_dim),
            (
        ...
        if self.p_emb_dim > 0:
            print("=========decoder (with p_emb)=========", self.decoder)
        else:
            print("=========decoder (without p_emb)=========", self.decoder)
        # Graph
        if "hybrid" in self.pert_model_type:
            ...
            for graph_type, g in graph.graph_dict.items():
                ...
                self.edge_index[graph_type] = torch.Tensor(edge_index)
                self.edge_weight[graph_type] = torch.Tensor(edge_weight)
    ...
    def forward(self, cntr, pert_idxs, p_emb):
        ...
        if self.no_pert_model:
            ...
            # Perturbation model
            self.edge_index = self.edge_index.to(z_intrinsic.device)
            self.edge_weight = self.edge_weight.to(z_intrinsic.device)
            ...
            self.pert_z = torch.zeros(batch_size, z_p.size(-1), device=z_p.device)
        ...
        # Decoder
        # print("p_emb.shape", p_emb.shape)
        # print("self.p_emb_dim", self.p_emb_dim)
        if p_emb.shape[1] > 2 and self.p_emb_dim > 0:
            prediction = self.decoder(
                torch.cat([z_intrinsic, self.pert_z, p_emb, cntr[:, self.output_dim :]], dim=-1)
            )
        else:
            prediction = self.decoder(
    ...
    def loss(
        ...
        """
        recon_loss = self.reconstruction_loss(prediction, target, mask_degs)
        ...
        return recon_loss + kl_div_intrinsic + bce_loss, logs

    def reconstruction_loss(self, x_hat, x, mask_degs):
        """
        Computes a hybrid reconstruction loss (MSE + cossim) between the predicted and the target expression.
        """
        if mask_degs is not None:
            # print("mask_degs.shape", mask_degs.shape, mask_degs)
            x_hat = x_hat[mask_degs]
            x = x[mask_degs]
