...
class PertPredictor(LightningModule):
    ...
    def loss(self, prediction, intrinsic_mean, intrinsic_log_var, target, mask_degs):

        target = target.to(next(self.parameters()).device)
        mask_degs = mask_degs.to(next(self.parameters()).device)
        
        loss, logs = self.model.loss(
            prediction, intrinsic_mean, intrinsic_log_var, target, mask_degs
        )
        self.log_dict(
            logs,
            prog_bar=True,
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss, logs = self.loss(
            ...
            mask_degs=batch.mask_degs
        )
        self.log("train/loss", loss, prog_bar=True, sync_dist=True)
        self.log("train/recon_loss", logs["recon_loss"], prog_bar=False, sync_dist=True)
    ...
    def sample_inference(self, cntr, pert_idxs, p_emb, cell_types):
        ...
        # if self.no_pert:
        #     return cntr
        # if self.basal_mode == "latent": # default
        #     zs = torch.randn(size, self.latent_dim, device=cntr.device)
        #     prediction = self.model.decoder(
        #         torch.cat(
        #             [zs + self.model.pert_z, p_emb, cntr[:, self.model.output_dim :]], dim=-1
        #         )
        #     )
